...
func TestMarshalJson(t *testing.T) {
	...
	tests := [...]struct {
		...
		{
			Args{"j": []string{"v1", "v2", "v3"}, "k": []string{"v1", "v2", "v3"}},
			strconv.Quote("j=v1;j=v2;j=v3;k=v1;k=v2;k=v3"),
		},
		{
			Args{"secret": []string{"yes"}},
			strconv.Quote("secret=yes"),
		},
		{
			Args{"sec;ret": []string{"n=o"}},
			strconv.Quote("sec\\;ret=n\\=o"),
		},
		{
			Args{"secret": []string{"nou"}, "cache": []string{"/tmp/cache"}},
			strconv.Quote("cache=/tmp/cache;secret=nou"),
		},
	}
	...
	for _, test := range tests {
		...
		if encodedstr != test.expected {
			t.Errorf("%q → %q (expected %q)", test.args, encodedstr, test.expected)
		}
		...
		if test.args != nil && !reflect.DeepEqual(args, test.args) {
			t.Errorf("%q → %q (expected equality)", test.args, args)
		}
	}
}
